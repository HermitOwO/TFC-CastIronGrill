...
public class StovetopCastIronGrillBlock extends BottomSupportedDeviceBlock implements IHighlightHandler
{
    ...
    private static final Map<Integer, VoxelShape> SLOT_RENDER_SHAPES = Map.of(
        1, Shapes.box(0.6, 0, 0.4, 0.8, 0.22, 0.6),
        0, Shapes.box(0.2, 0, 0.4, 0.4, 0.22, 0.6)
    );
    ...
    private static final VoxelShape GRILL_SHAPE = box(1, 0, 1, 15, 1, 15);

    public StovetopCastIronGrillBlock(ExtendedProperties properties)
    {
    ...
    public void animateTick(BlockState state, Level level, BlockPos pos, RandomSource rand)
    {
        ...
        if (level.getBlockEntity(pos) instanceof StovetopCastIronGrillBlockEntity grill && grill.getTemperature() > 0f)
        {
    ...
    {
        if (level.getBlockEntity(pos) instanceof StovetopCastIronGrillBlockEntity grill)
        {
            final var inv = grill.getInventory();
    ...
    {
        if (!entity.fireImmune() && entity instanceof LivingEntity && level.getBlockEntity(pos) instanceof StovetopCastIronGrillBlockEntity grill && grill.getTemperature() > 0)
        {
